...
public class MerchantServiceImpl implements MerchantService {
  ...
  public Page<Merchant> queryAll(Merchant merchant) {
    ...
    List<Merchant> merchantList = pages.getResult();
    merchantList.forEach((v) -> {
      Coupon c = couponMapper.queryAllByLimit(0, 1).get(0);
      v.setCoupon(c);
    });
    Page<Merchant> page = (Page<Merchant>) merchantList;
    return page;
  }
}